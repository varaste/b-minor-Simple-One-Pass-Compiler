...
//	int snum[50];
//	int n, j;
//	for (int i=0; i<50; i++){   //geting input
//		printf("\nEnter number:");
//		scanf("%d",&n);
//		snum[i]=n;		
//	}
//	
//	for (j = 0; j < 50; j++){     // bubble sort 
//		if (snum[j] > snum[j+1]){
//			swap(&snum[j], &snum[j+1]); 
//		} 
//	}
//	  
...
//    } 
//	
//int binarySearch(int snum[], int l, int r, int x) { 
//    if (r >= l) { 
//        int mid = l + (r - l) / 2; 
//        if (snum[mid] == x) 
//            return mid; 
//
//        if (snum[mid] > x) 
//            return binarySearch(snum, l, mid - 1, x); 
//        return binarySearch(snum, mid + 1, r, x); 
//    } 
//    return -1;
//}
