...
// Token Enum
enum t_token {
    NONE, DONE, NUMBER, IDENTIFIER, KEYWORD, DELIMITER, OPERATOR, CHAR, STRING
};
...
int symbolLookup(char str[]) {
    for (int i = 0; i < sym_count; i++) {
        if (strcmp(str, sym_table[i]) == 0) return i;
    }
...
int testGeneral(char c, char charset[]) {
...
void printToken(int *tokenValue, enum t_token *tokenType) {
    switch (*tokenType) {
        ...
        case DELIMITER:
            if (*tokenValue < 900) {
                printf("%c          NONE\n", (char) *tokenValue);
            } else if (*tokenValue == INC_OP) printf("++         NONE\n");
            else if (*tokenValue == DEC_OP) printf("--         NONE\n");
            else if (*tokenValue == EQ_OP) printf("==         NONE\n");
            else if (*tokenValue == GE_OP) printf(">=         NONE\n");
            else if (*tokenValue == LE_OP) printf("<=         NONE\n");
            else if (*tokenValue == NE_OP) printf("!=         NONE\n");
            else if (*tokenValue == AND_OP) printf("&&         NONE\n");
            else if (*tokenValue == OR_OP) printf("||         NONE\n");
            break;
    }
}

int main(int argc, char **argv) {
//    address file ro inja mizarim
    fp = fopen("C:\Users\Arya Varaste\Desktop\CD\HW\3\B-Mimor Compiler\testcase\test2.b", "r");
    printf("%s started compiling %s ... %s\n", YEL, "/Users/reza/CLionProjects/b_minor/test3.txt", NC);
